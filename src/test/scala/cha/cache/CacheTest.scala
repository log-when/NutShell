...
class CacheStage1Prop(implicit val cacheConfig: CacheConfig) extends CacheModule{
  ...
  chaAssume(this,"cpuReqValid -> ((cpuReqValid U coreReqFire) || G cpuReqValid)")
  ...
  // inducted by s2_assert_1, s3_assert_1, s3_assert_2, array_assert_1
  ...
  // inducted by s2_assert_3, s3_assert_1, s3_assert_2, array_assert_1
  chaAssume(this,"(G !s1ReqFire) -> F G arrayReadReqReady")

  val dataArrayReadValid = cio.dataReadBus.req.valid
  // s1_assert_1: if request to s2 does not make a handshake, then there will be no read request to dataArray
...
class CacheStage2Prop(implicit val cacheConfig: CacheConfig) extends CacheModule{
  ...
  // s2_assert_2: request to s3 will keep high until getting handshake
  ...
  // s2_assert_3: if request from s1 does not make a handshake, there will be no request to s3 or s3 is always unready
  // no_deps, can be proven
  // chaAssert(this, "(G !s1ReqFire) -> ((F G !s2ReqValid) || G !s2ReqReady)")

  // s2_assert_4: if request from s2 does not make a handshake, request from s1 will not make a handshake eventually
  // no_deps, can be proven
  ...
  // s2_assert_5: if request to s3 is valid and the request meets a hit, then the output tag is the same as its in request 
  // no_deps, can be proven
...
class CacheStage3Prop(implicit val cacheConfig: CacheConfig) extends CacheModule{
  ...
  val cpuRespValid = cio.in.resp.valid
  ...
  chaAssume(this, "(G !cpuRespValid) -> F G dataArrayReadReady")
  ...
  // chaAssert(this, "(G !cpuRespValid) -> F G !s2ReqFire")

  // s3_assert_4: if no response to cpu, there will be no request to write metaArray and dataArray
  // take all the assumptions, can be proven
  // chaAssert(this, "(G !cpuRespValid) -> F G !arrayWriteReqValid")
...
class CacheArrayProp(implicit val cacheConfig: CacheConfig) extends CacheModule{
  ...
  // array_assert_1: if there is always no write request to array,  array will be ready to read eventually
  // no deps, can be proven 
...
class CacheSafetyProp extends Cache()(CacheConfig()){
...
class CachePropSpec extends AnyFlatSpec with ChiselScalatestTester with Formal {
  ...
  // it should "pass" in {
  //   verify(new CacheSafetyProp, Seq(BoundedCheck(10), PonoEngineAnnotation, EnSafetyOpti))
  // }
}